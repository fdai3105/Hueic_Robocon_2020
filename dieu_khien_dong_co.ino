void dieu_khien_dong_co(int huong_cb, int huong_robot, int toc_do_trai, int toc_do_phai)
{
  ...
  if (huong_robot == chay_toi) {
    if (huong_cb == cb_truoc_sau) {
      ...
      analogWrite(pwm2_toi, PID_phai);
      analogWrite(pwm4_toi, PID_phai);
    }
  }

  if (huong_robot == chay_lui) {
    if (huong_cb == cb_sau_truoc) {
      analogWrite(pwm1_lui, PID_phai_1);
      analogWrite(pwm3_lui, PID_phai);
      analogWrite(pwm2_lui, PID_trai_1);
      analogWrite(pwm4_lui, PID_trai);
